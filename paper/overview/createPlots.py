...
if __name__ == '__main__':
	...
	map_filename = "instance.yaml"
	with open(map_filename) as map_file:
		map_data = yaml.load(map_file, Loader=yaml.SafeLoader)

	data = np.load("central.npy")
	colors = plot_state_space(map_data, data)
	plt.axis('off')
	plt.savefig("central.svg", bbox_inches='tight')

	dataset = get_observations(map_data, data, sampling=100)
	for agent, t, obs in dataset:
		fig = plot_observation(obs, color=colors[agent])
		plt.axis('off')
		plt.savefig("obs_t_{}_agent_{}.svg".format(t, agent), bbox_inches='tight')
