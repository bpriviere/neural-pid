...
if __name__ == "__main__":
  ...
  if args.plot:
    ...
    # plot loss curve
    pp = PdfPages("exp1_loss_di.pdf")
    fig,ax = plt.subplots()    
   
    for solver in solvers.keys():
      files = glob.glob("doubleintegrator/{}*/*.csv".format(solver), recursive=True)
      if len(files) == 0:
        continue
  ...
  for i in range(0,1):
      ...
      if args.train:
        for cc in ['Empty', 'Barrier']:
          ...
          # if cc == 'Barrier':
          #   param.il_pretrain_weights_fn = 'doubleintegrator/exp1Empty_{}/il_current.pt.tar'.format(i)
          #   param.il_lr /= 10 
        ...
        controller1 = "exp1Empty_{0},EmptyAPF,../results/doubleintegrator/exp1Empty_{0}/il_current.pt".format(i)
