...
def add_line_plot_agg(pp,result_by_instance,key,title=None, x_label=None, y_label=None, group_by="num_agents",\
	ax=None, aggregrate_successful_agent=False):
	if ax is None:
		fig,ax = plt.subplots()
			...
			for instance,results in result_by_instance.items():
				...
				for r in results:
					if r[group_by] == x and r["solver"] == solver:
						...
						# print(instance)
						# print(r[key])
						if r["num_model"] not in num_models:
							curr[r["num_model"]] = 0
							curr_count[r["num_model"]] = 0
...
if __name__ == '__main__':
	...
	# add_bar_agg(pp, result_by_instance, "num_collisions", "# collisions")
